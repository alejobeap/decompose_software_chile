...
for ii = 1:nframes
    ...
    if par.grd_ref_gnss_los == 1
      disp(['Saving LOS reference GNSS tiffs'])
      if ~isfolder([par.out_path, 'TIFS'])
        mkdir([par.out_path, 'TIFS'])
      end
      %disp(['Saving LOS reference GNSS tiffs'])
% crop to minimise file size
      x = xx(1,:); y = yy(:,1);
      [~,x_ind,y_ind,x_crop,y_crop] = crop_nans(vel(:,:,ii),x,y);
        
        % create georeference
      georef = georefpostings([min(y_crop) max(y_crop)],...
            [min(x_crop) max(x_crop)],size(vel(y_ind,x_ind,:)),'ColumnsStartFrom','south',...
            'RowsStartFrom','west');    
        
        % write outputs
      geotiffwrite([par.out_path 'TIFS' filesep 'LOS_ref_GNSS' num2str(ii) '.geo.tif'],vel(y_ind,x_ind,ii),georef)
...
if par.grd_ref_gnss_los == 1
    ...
    for ii = 1:length(insar_id)
        LOS = gnss_los(:, : ,ii);
	    x = xx(1,:); y = yy(:,1);
        [~,x_ind,y_ind,x_crop,y_crop] = crop_nans(gnss_los(:,:,ii),x,y);
        ...
        % write outputs
        fprintf('%.0f/%.0f Writing TIFS LOS for %s to .tif...\n', ii, length(insar_id), insar_id{ii})
        geotiffwrite([par.out_path 'TIFS' filesep insar_id{ii} '_GNSS_LOS.tif'],gnss_los(y_ind,x_ind,ii),georef)
        %geotiffwrite([par.out_path 'TIFS' filesep insar_id{ii} '_GNSS_LOS.tif'],LOS(ylims(1):ylims(2),xlims(1):xlims(2),georef)
    end
    %geotiffwrite([par.out_path 'TIFF' filesep 'GNSS_E.tif'],(xx(1, :), yy(:, 1), gnss_E(:,:)),georef)
    %geotiffwrite([par.out_path 'TIFF' filesep 'GNSS_N.tif'],(xx(1, :), yy(:, 1), gnss_N(:,:)),georef)
